...
app.config['MYSQL_PASSWORD']="manjay1234"
app.config['MYSQL_DB']="manjay"
mysql=MySQL(app)
...
def userlogin():
    ...
    if request.method=='POST':
        ...
        cursor.execute('SELECT * FROM user WHERE usermail = %s AND userpassword = %s', (useremail, userpassword))
        user=cursor.fetchone()
        if user:
            session['loggedin'] = True
            session['u_psw'] = user['userpassword']
...
def userregistration():
    ...
    if request.method == 'POST':
        ...
        if account:
            ...
            cursor.execute('INSERT INTO user VALUES (%s, %s, %s)', (username, usermail, userpassword))
...
def product():
    if request.method=='POST':
        ...
        cursor=mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('select * from products where name=%s',(keyval,))
...
def doctorlogin():
    if request.method=='POST':
        ...
        cursor=mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT * FROM doctors WHERE adminemail = %s AND adminpassword = %s', (adminname, adminpassword))
...
def doctorregistration():
    if request.method=='POST':
        ...
        adminpassword=request.form['admin_psw']
        cursor=mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        account=cursor.execute('SELECT * FROM doctors WHERE adminemail=%s',(adminemail,))
        if account :
...
def docmain():
    if 'loggedin' in session:
        ...
        cursor.execute('select d_days from docprofile where d_email=%s',(demail,))
...
def profileupdate():
    if request.method=='POST':
            ...
            cursor=mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cursor.execute('INSERT INTO docprofile VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(dname,dage,demail,dphno,dphno2,ddegree,dspeci,dclg,drank,dexp,dhosp,dtreat,dfees,ddays,dtiming,dstatus,dmeet))
            mysql.connection.commit()
